...
const StepCard = ({ step, title, details }: { step: number, title: string, details: string }) => {
    return (
        <div className="border-yellow-300 border-2 rounded-lg flex flex-col w-[200px] h-[200px] lg:w-[250px] lg:h-[250px] p-4 gap-y-2 shadow-[6px_6px_6px_6px_rgb(253,224,71,80)]">
            <div className='text-xl text-yellow-300'>{step}</div>
            <div className='text-xs'><span className='text-yellow-300'>{title} </span> – {details}</div>
        </div>
    )
}
const HowItWorks = () => {
